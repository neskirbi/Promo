...
function clientes_hoy($fecha,$usuario,$conexion)
{
  ...
  {
    $sql1="SELECT Id_cliente FROM clientev where ((cl_email='".$sql_obj->fbise."' or cl_email='".$sql_obj->fsem."' or cl_email='".$sql_obj->fmen."') and cl_fechaalta <='".$fecha."' and ".$_dias[$num_dia]."='1' and Id_ruta='".$usuario."' ) order by cl_razonsocial";
...
function dnis($usu,$conexion)
{
  ...
  $sql="SELECT  *  FROM Supervisor WHERE Gerencia='$usu' or Ejecutivo='$usu' or Monitoreo='$usu' or Nombre='$usu' or susu='$usu'  or susu1='$usu'  or susu2='$usu' or susu3='$usu' order by Ruta asc ";
  $sql=odbc_exec($conexion, $sql);
  return $sql;
}
